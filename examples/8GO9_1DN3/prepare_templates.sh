...
cd $AF_UNMASKED_DIR
#python prepare_templates.py --target $FASTAFILE --template $TEMPLATES/pdb8go9.ent --output_dir $OUTDIR --align
#python prepare_templates.py --target $FASTAFILE --template $TEMPLATES/pdb1dn3.ent  --output_dir $OUTDIR --align --append

#python prepare_templates.py --target $FASTAFILE --template $TEMPLATES/pdb8go9.ent --output_dir $OUTDIR --align
#python prepare_templates.py --target $FASTAFILE --template $TEMPLATES/pdb1dn3.ent  --output_dir $OUTDIR --align --append

python prepare_templates.py --target $OUTDIR/$NAME/8GO9.fasta \
    --template $TEMPLATES/AF-O00590-F1-model_v4.pdb \
    --output_dir $OUTDIR \
    --align

python prepare_templates.py --target $OUTDIR/$NAME/1DN3.fasta \
    --template $TEMPLATES/AF-P02776-F1-model_v4.pdb \
    --output_dir $OUTDIR \
    --align \
    --append

# python prepare_templates.py --target $TEMPLATES/AF-O00590-F1-model_v4.pdb \
#     --template $TEMPLATES/AF-P02776-F1-model_v4.pdb \
#     --output_dir $OUTDIR \
#     --superimpose
